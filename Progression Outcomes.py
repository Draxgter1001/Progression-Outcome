...
def studentid_format(student_id):           # Custom function to check if the student ID entered is in the correct format
    character_checker = 'w'
...
def progression_outcome(x, y ,z):           # Custom function used to predict the outcomes which are used for part 1 and the other parts.
    if x == 120:
        print('Progress\n')
...
def stars_print():              # Custom function used to print the number of stars based on the number of predicted outcomes (For part 1)
    stars_progress = 0
    stars_trailer = 0
    stars_retriever = 0
    stars_exclude = 0
...
def print_dictionary():         # Custom function that print the student ID and its credits (Part 4)
...
def write_txt_file():           # Custom function used to create the text file (For part 3)
    with open('Part3.txt', 'w') as write_txt:
        for i in range(len(outcomes)):
            data_set = str(outcomes[i]) + ' - ' + str(final_output[i]) + '\n'
            write_txt.write(data_set)

def read_text_file():           # Custom function used to print the Histogram (For part 1)
    with open('Part3.txt', 'r') as file:
        print(file.read())

def histogram():            # Custom function used to print the Histogram
    print("\n-----------------------------------------------------------")
    print('Histogram')
    print('\nPart 1')
    stars_print()
...
while(True):
    ...
    string_format = f"{pass_credits}, {defer_credits}, {fail_credits}"          # Custom function used to print the Histogram (For part 1)
